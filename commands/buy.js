...
module.exports = {
  ...
  run: async (client, interaction, db) => {
    try {
      ...
      const collector = await r4ayvQXBrbPKg4dnFc1q6wsVPBMQivyKgi filter, time: 0 });
      ...
      collector.on('collect', async i => {
        ...
        interaction.channel.awaitMessages({ filter, max: 1, time: 1000 * 60, errors: ['time'] })
          .then(async collected => {
            var numberOfAccs = collected.first();
            ...
            if (Number(data.acc.length) >= Number(collected.first().content)) {
              ...
              console.log(numberOfAccs.content)
              interaction.editReply({
                content: `⚠️ **Please Make Sure The Probot Lang Is English! and your dm is opend** ⚠️\nYou Have 2m To Type: \`#credits ${transID} ${data.price * numberOfAccs.content}\``,
                allowMention: {
                  repliedUser: false
                },
                ephemeral: true,
              ...
              const result = tax(data.price * Number(numberOfAccs.content));
              const filter = i => i.author.id == botID && i.content.startsWith(`**:moneybag: | ${interaction.user.username}, has transferred `) && i.content.includes(Number(data.price) * Number(numberOfAccs.content) - Number(result.protax));
              console.log(result.protax)
      ...
      interaction.editReply({
        content: `**the bot got an error on his face**`,
        allowMention: {
          repliedUser: false
        },
        ephemeral: true,
      ...
      interaction.channel.send('the bot got an error on his face').catch(() => { })
